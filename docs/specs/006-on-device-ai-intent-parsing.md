...
| 2026-07-23 | Claude Code | Real-device testing showed the model matched only near-exact phrasing and repeated the small clarification pool often. Added `StoryOption.hints`/`EngineOption.hints` (alternate phrasings, matching-only, never shown in UI) with hints authored into `story.json`; added `HeuristicIntentMatcher` (deterministic word-overlap pre-pass, no AI) tried before Foundation Models; single-valid-option nodes now match directly (no real ambiguity to protect against); loosened the AI prompt from "only if clearly obvious" to "closest sense, even if wording differs" with a few-shot example; `ClarificationMessages.random(excluding:)` avoids repeating the immediately-previous line. Added `HeuristicIntentMatcherTests` (fully deterministic, no device needed) and extended `FoundationModelsIntentParserTests`/`ChatViewModelTests`. |
| 2026-07-23 | Claude Code | Follow-up real-device testing showed the previous loosening overcorrected: the model started force-matching topically-unrelated input (e.g. "corre" matched to "tem alguma saída perto?") instead of returning nil. Split the prompt guidance into two explicit axes — divergent *phrasing* of the same topic (accept) vs. a genuinely different *topic* (reject, even if thematically adjacent) — with one positive and one negative few-shot example. Also added an explicit "ignore trailing '?'" instruction (already true structurally in `HeuristicIntentMatcher`, since punctuation is stripped during normalization). |
