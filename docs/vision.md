...
Three emergent endings:

1. **Escape, transformed** — she gets out, but not intact; she leaves carrying what happened.
2. **Surrender** — sanity reaches 0; she stops trying to escape, and her messages degrade into
   symbols until the chat simply stops.
3. **Taken** — something outside comprehension reaches her mid-escape. She cannot describe it,
   and then she is gone; the chat answers nothing after that.

Endings are a **function of accumulated state** (sanity, clues discovered, city rules respected
or violated) — **never** a direct "pick ending X" choice, and never random.

> **Implemented in Spec 004, reworked in Spec 008:** `sanity` and boolean flags are live and
> gate which options the engine exposes at each node. The `trust` variable was removed in Spec
> 008 (it hurt the experience), and the original five ending archetypes were collapsed into the
> three above to fit the available development time.
...
> scoped out of v1 but have since shipped — see Specs 005, 006, and 007.
...
| 004 | State Variables | Sanity, trust, conditional flags (trust later removed in 008) |
| 005 | Persistence | Save/load progress with SwiftData |
| 006 | On-Device AI — Intent Parsing | Free-text input + Foundation Models intent matching |
| 007 | Dynamic Narration | Character voice generated by Foundation Models |
| 008 | Sanity Rework + Three Endings | Remove trust, soften sanity, 5 endings → 3, debug meter |
| 009 | Game Loop & Redundancy | Inspect actions, revisit text, exploration loops |
| 010+ | Menu + Achievements | Main menu, continue flow, endings gallery |
| 011+ | App Intents / Siri | Apple Intelligence integration, Shortcuts |
